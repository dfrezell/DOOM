    ...
    if (grabMouse)
    {
        if (!--doPointerWarp)
        {
            XWarpPointer(X_display,
                         None,
                         X_mainWindow, 0, 0, 0, 0, X_width / 2, X_height / 2);
...
void I_FinishUpdate(void)
{
        ...
        while (y--)
        {
            ...
            do
            {
                ...
                fouropixels[2] = ((fouripixels << 16) & 0xffff0000)
                        | ((fouripixels << 8) & 0xff00) | (fouripixels & 0xff);
#ifdef __BIG_ENDIAN__
    ...
    if (doShm)
    {
        ...
        if (!XShmPutImage(X_display,
                          X_mainWindow,
                          X_gc, image, 0, 0, 0, 0, X_width, X_height, True))
    ...
    else
    {
        ...
        // draw the image
        XPutImage(X_display,
                  X_mainWindow, X_gc, image, 0, 0, 0, 0, X_width, X_height);
...
void I_InitGraphics(void)
{
    ...
    X_mainWindow = XCreateWindow(X_display, RootWindow(X_display, X_screen), x, y, X_width, X_height, 0,    // borderwidth
                                 8, // depth
    ...
    if (doShm)
    {
        ...
        image = XShmCreateImage(X_display,
                                X_visual,
                                8, ZPixmap, 0, &X_shminfo, X_width, X_height);
