...
void
        ...
        V_DrawBlock(int x, int y, int scrn, int width, int height, byte * src) {
    byte           *dest;

#ifdef RANGECHECK
    if (x < 0
        || x + width > SCREENWIDTH
        || y < 0 || y + height > SCREENHEIGHT || (unsigned)scrn > 4)
    {
...
void
        ...
        V_GetBlock(int x, int y, int scrn, int width, int height, byte * dest) {
    byte           *src;
